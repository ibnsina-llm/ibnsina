"""
Qwen3-shaped model for nanochat's training loop. Select with NANOCHAT_ARCH=qwen3 (see nanochat/arch.py).
Exactly the llama.cpp / HF `qwen3` architecture so checkpoints export to standard GGUF:
- pre-norm RMSNorm with learnable gain (attn_norm / ffn_norm / output_norm)
- per-head QK-norm: RMSNorm (learnable gain of size head_dim) on Q and K after the head reshape, BEFORE RoPE
  (HF Qwen3's q_norm/k_norm; llama.cpp loads them as blk.N.attn_q_norm / attn_k_norm) — bounds attention logits;
  the 1.5B (Llama arch, no QK-norm) hit max |q·k/sqrt(d)| = 102 by step 27k
- RoPE, textbook rotation (+theta) on half-split channels (HF layout == llama.cpp NEOX mode, so NO Q/K permute at export)
- MHA/GQA (n_kv_head), scale 1/sqrt(head_dim), full causal context, no attention bias (Qwen3 dropped Qwen2's QKV bias)
- SwiGLU MLP (gate/up/down), hidden = round_up(8/3 * n_embd, 256)
- untied token embedding and lm_head; no logit softcap, no value embeddings, no lambdas, no sliding windows
...
class Qwen3Config:
    ...
    n_kv_head: int = 6
    n_embd: int = 768
    window_pattern: str = "L"   # accepted for CLI/meta compatibility; a Qwen3 block always attends to the full context
    arch: str = "qwen3"
...
def apply_rotary_emb(x, cos, sin):
    """Textbook RoPE on half-split channels (HF convention): (x1, x2) -> (x1 cos - x2 sin, x1 sin + x2 cos)."""
    assert x.ndim == 4
    d = x.shape[3] // 2
    x1, x2 = x[..., :d], x[..., d:]
...
class Attention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.q_norm = RMSNorm(self.head_dim, config.norm_eps)  # Qwen3 QK-norm: per-head, shared across heads/positions
        self.k_norm = RMSNorm(self.head_dim, config.norm_eps)

    def forward(self, x, cos_sin, window_size, kv_cache):
        B, T, C = x.size()
        q = self.q_norm(self.c_q(x).view(B, T, self.n_head, self.head_dim))       # QK-norm after head reshape, before RoPE
        k = self.k_norm(self.c_k(x).view(B, T, self.n_kv_head, self.head_dim))
...
class Qwen3(nn.Module):
    ...
    def init_weights(self):
        ...
        torch.nn.init.normal_(self.transformer.wte.weight, mean=0.0, std=1.0)   # RMS ~1 residual stream (nanochat normalises embeddings; Qwen3 has no post-embedding norm)
        ...
        for block in self.transformer.h:
            ...
            torch.nn.init.ones_(block.attn.q_norm.weight)
            torch.nn.init.ones_(block.attn.k_norm.weight)
    ...
    def _block_norms(self, b):
        return (b.attn_norm.weight, b.mlp_norm.weight, b.attn.q_norm.weight, b.attn.k_norm.weight)

    def _norm_params(self):
        return [self.norm_f.weight] + [p for b in self.transformer.h for p in self._block_norms(b)]
    ...
    def num_scaling_params(self):
        ...
        transformer_matrices = sum(p.numel() for p in self.transformer.h.parameters()) - sum(p.numel() for b in self.transformer.h for p in self._block_norms(b))
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02, weight_decay=0.0, scalar_lr=0.5):
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=norm_params, lr=scalar_lr * 0.02, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0),  # RMSNorm gains incl. q_norm/k_norm
        ]
