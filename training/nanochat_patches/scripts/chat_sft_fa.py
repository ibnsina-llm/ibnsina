...
from tasks.persian_sft import PersianSFT
...
parser.add_argument("--sft-dir", type=str, default="/data/sft/v1", help="dir with train.jsonl, val.jsonl, smoltalk_fa.jsonl")
parser.add_argument("--mmlu-rows", type=int, default=50000, help="rows of MMLU auxiliary_train (MIT) as multiple-choice format teacher")
parser.add_argument("--smoltalk-fa-epochs", type=int, default=8, help="epochs of the translated SmolTalk conversations in the mixture")
...
train_tasks = [
    PersianSFT(os.path.join(args.sft_dir, "train.jsonl")),  # FarsInstruct permissive subsets
    *([PersianSFT(os.path.join(args.sft_dir, "smoltalk_fa.jsonl"), epochs=args.smoltalk_fa_epochs)] if os.path.exists(os.path.join(args.sft_dir, "smoltalk_fa.jsonl")) else []),  # translated SmolTalk (v1 layout only; v2 bakes its share into train.jsonl)
    MMLU(subset="all", split="auxiliary_train", stop=args.mmlu_rows),  # MIT; teaches the letter-answer format used by the evals
]
train_dataset = TaskMixture(train_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows (FarsInstruct + SmolTalk-fa x{args.smoltalk_fa_epochs} + MMLU aux {args.mmlu_rows})")
_val = os.path.join(args.sft_dir, "val.jsonl")
if not os.path.exists(_val): _val = os.path.join(args.sft_dir, "sft_eval.jsonl")   # v2 layout
val_dataset = TaskMixture([PersianSFT(_val)])
    ...
    if last_step:
        ...
        save_checkpoint(
            ...
            optimizer.state_dict(),
            {
                ...
                "model_config": {**meta.get("model_config", {}), "sequence_len": args.max_seq_len},  # carry the FULL base config (arch, ffn_hidden, rope_base, ...) so eval/export rebuild the right model
